...
def plot_gp_surfaces_mpl(all_X, all_y, gps, top_dims, fns=range(1, 9),
                          n_grid=40, figsize_scale=4.2, cmap_name='rainbow'):
    """Matplotlib version of plot_gp_surfaces that returns matplotlib figure."""
    fns = list(fns)
    ncols = min(4, len(fns)); nrows = -(-len(fns) // ncols)
    ...
    for i, fn in enumerate(fns):
        ...
        ax.plot_surface(G1, G2, Z, cmap=cmap, alpha=0.85, linewidth=0, antialiased=True)
        xo, yo, yr = X[:, j1], X[:, j2], Yraw
        ax.scatter(xo, yo, yr, c=yr, cmap=cmap, s=25, edgecolors='black', linewidths=0.5)
        ax.set_xlabel(d1n, fontsize=8, labelpad=2)
        ax.set_ylabel(d2n, fontsize=8, labelpad=2)
        ax.set_zlabel('y', fontsize=8, labelpad=2)
        ax.set_title(f"F{fn} — ({d1n},{d2n})", fontsize=10)
        ax.tick_params(labelsize=7)

    fig.suptitle("GP mean-surface slices — top-2 dims per function, rest fixed at incumbent",
                 fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    return fig


def _de_climb_setup(all_X, all_y, gps, diagnostics, fns=None, acquisition='EI', n_grid=35):
    """Shared setup for the two DE-climbing plots below. Keeps the static plot and the GIF maker
    ...
    for fn in fns:
        ...
        def sc(f, acquisition=acquisition, gp=gp, ym=ym, ys=ys, active=active, Yw=Yw):
            m, s = gp.predict(f, return_std=True)
            if acquisition == 'EI':
                return aEI(m, s, ym, ys)
            elif acquisition == 'UCB':
    ...
    fns = [fn for fn in fns if fn in traces]
    return fns, traces, dims, surfaces


def plot_de_climbing_mpl(all_X, all_y, gps, diagnostics, fns=None, acquisition='EI',
                          n_grid=35, generation='last', figsize_scale=4.2, cmap_name='rainbow'):
    """Matplotlib version of plot_de_climbing_interactive, showing one
    generation as a static plot"""
    fns, traces, dims, surfaces = _de_climb_setup(all_X, all_y, gps, diagnostics, fns, acquisition, n_grid)
    if not fns:
        print("No function is currently routed to 'global' -- nothing to trace.")
        return None
    ...
    for i, fn in enumerate(fns):
        G1, G2, Z = surfaces[fn]
        ai1, ai2, d1n, d2n = dims[fn]
        gen_idx = (len(traces[fn]) - 1) if generation == 'last' else min(generation, len(traces[fn]) - 1)
        h = traces[fn][gen_idx]
        pop = h['population']; energies = -h['energies']

        ax = fig.add_subplot(nrows, ncols, i + 1, projection='3d')
        ax.plot_surface(G1, G2, Z, cmap=cmap, alpha=0.55, linewidth=0, antialiased=True)
        ax.scatter(pop[:, ai1], pop[:, ai2], energies, c='#E91E63', s=30,
                   edgecolors='black', linewidths=0.4)
        ax.set_xlabel(d1n, fontsize=8, labelpad=2)
        ax.set_ylabel(d2n, fontsize=8, labelpad=2)
        ax.set_zlabel(f'{acquisition} score', fontsize=8, labelpad=2)
        ax.set_title(f"F{fn} — ({d1n},{d2n}), gen {gen_idx+1}/{len(traces[fn])}", fontsize=10)
        ax.tick_params(labelsize=7)

    which = "final, converged generation" if generation == 'last' else f"generation {generation+1}"
    fig.suptitle(f"DE climbing the {acquisition} acquisition surface — {which}", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    return fig


def save_de_climbing_gif_mpl(all_X, all_y, gps, diagnostics, fns=None, acquisition='EI',
                              n_grid=35, out_path='de_climbing.gif', stride=2, fps=8,
                              figsize_scale=4.0, cmap_name='rainbow'):
    """Renders the full DE-climbing history"""
    fns, traces, dims, surfaces = _de_climb_setup(all_X, all_y, gps, diagnostics, fns, acquisition, n_grid)
    if not fns:
        print("No function is currently routed to 'global' -- nothing to trace.")
        return None
    ...
    for i, fn in enumerate(fns):
        ...
        ax.tick_params(labelsize=7)
        axes[fn] = ax
        scat_artists[fn] = ax.scatter([], [], [], c='#E91E63', s=30, edgecolors='black', linewidths=0.4)

    suptitle = fig.suptitle(f"DE climbing the {acquisition} acquisition surface", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.94])

    gen_indices = list(range(0, n_gen_max, stride))
    if gen_indices[-1] != n_gen_max - 1:
        gen_indices.append(n_gen_max - 1)
    ...
    def update(gen_idx):
        ...
        for fn in fns:
            ...
            ai1, ai2, *_ = dims[fn]
            pop = h['population']; energies = -h['energies']
            scat_artists[fn].remove()
            scat_artists[fn] = axes[fn].scatter(pop[:, ai1], pop[:, ai2], energies,
                                                 c='#E91E63', s=30, edgecolors='black', linewidths=0.4)
            artists.append(scat_artists[fn])
        suptitle.set_text(f"DE climbing the {acquisition} acquisition surface  |  "
                           f"generation {gen_idx+1}/{n_gen_max}")
        return artists

    ani = animation.FuncAnimation(fig, update, frames=gen_indices, blit=False)
    ani.save(out_path, writer=animation.PillowWriter(fps=fps), dpi=90)
    plt.close(fig)
    print(f"Saved: {out_path}")